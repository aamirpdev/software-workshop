...
import { decidePaymentStatus } from '../../../helpers';
import { PageWrapper } from './InternDashboard.style';

import checkOutstandingInstallments from '../../../helpers/checkOutstandingInstallments';
  ...
  const fetchData = async () => {
    try {
      ...
      setState({
        ...
        nextBooking,
        reviews,
        installments: checkOutstandingInstallments(installments),
        paymentsHistory,
        profileCompleted,
      });
    } catch (err) {
        ...
        <Col w={[4, 10, 8]} mb={bottomMargins.col[device]}>
          ...
          {nextBooking && Object.keys(nextBooking).length > 0 ? (
            <BookingCards
              ...
              interests={nextBooking.interests}
              status={nextBooking.status}
            />
          ) : (
            <T.P color={colors.gray} mb="5">
              No current booking
            </T.P>
          )}
        </Col>
