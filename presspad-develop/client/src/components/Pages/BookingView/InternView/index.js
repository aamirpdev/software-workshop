...
import { H5C, H6C } from '../../../Common/Typography';
...
export default class BookingView extends Component {
  ...
  render() {
    const { bookingInfo, role, id, bookingReviews, windowWidth } = this.props;
    ...
    const hostInfo = {
      ...
      gender,
      'university_/_school': lastStudyUniversity,
      hometown,
    ...
    if (installments[0]) {
      ...
      payedAmount = calculatePaidSoFar(installments);
    }
      ...
      <Wrapper>
        ...
        <ContentWrapper>
          <Title color="blue" pageTitle windowWidth={windowWidth} inner>
            {decideHeadline(bookingStatus.status)}
          </Title>
        ...
        {(status === 'accepted' || status === 'confirmed') && (
          <BioReportSection
            ...
            role={role}
            id={HOST_PROFILE.replace(':id', bookingInfo.host._id)}
            bio={bio}
