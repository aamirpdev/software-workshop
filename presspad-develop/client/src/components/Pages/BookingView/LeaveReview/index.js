...
const reducer = (state, action) => {
  switch (action.type) {
    ...
    case 'reviewAboutMe':
      return {
        ...state,
        reviewAboutMe: action.message,
        ratingAboutMe: action.rating,
      };
    default:
      throw new Error();
  }
...
const LeaveReview = ({
  ...
  useEffect(() => {
    const userReviews = reviews.filter(review => review.from === userId);
    const reviewsAboutMe = reviews.filter(review => review.to === userId);
    if (userReviews[0]) {
      dispatch({ type: 'submitedReview', value: userReviews[0].message });
      dispatch({ type: 'changeRate', value: userReviews[0].rating });
    }
    if (reviewsAboutMe[0]) {
      dispatch({
        type: 'reviewAboutMe',
        message: reviewsAboutMe[0].message,
        rating: reviewsAboutMe[0].rating,
      });
    }
  }, [reviews, userId]);
  ...
  const handleSubmit = async () => {
    if (text.length < 50) {
      ...
      try {
        ...
        message.success('Submited review succesfully').then(() => {
          dispatch({ type: 'submitedReview', value: data.message });
          dispatch({ type: 'changeRate', value: data.rating });
        });
      } catch (err) {
        const errorMsg =
  ...
  return review ? (
    <Wrapper>
      ...
      <PBold mb="2">Your review:</PBold>
      <P mb="3">{review || 'N/A'}</P>
      <Rate allowHalf value={rating} character={<Star />} disabled />
      {reviewAboutMe && (
        <>
          <PBold mb="2" mt="3">
            Their review:
          </PBold>
          <P mb="3">{reviewAboutMe || 'N/A'}</P>
          <Rate allowHalf value={ratingAboutMe} character={<Star />} disabled />
        </>
      )}
    </Wrapper>
  ) : (
    <Wrapper>
      ...
      {userRole === 'host' ? (
        <P>
          ...
          Once you have left your review you will also then be able to see any
          review they have left for you.
        </P>
      ) : (
        <P>
          Now that {toFirstName}’s stay has finished, we’d love you to leave a
