...
const AcceptedContent = ({ internName, bookingInfo, windowWidth }) => {
  const { price } = bookingInfo;
  // if booking is free and host accepts -> render confirmed
  if (price === 0) {
    return (
      <>
        <Title color="blue" pageTitle windowWidth={windowWidth} inner>
        ...
        <T.H6C mb="2" mt="5" color="lightGray">
          status
        </T.H6C>
        <T.H5C color="blue">
          confirmed <FireWorksImg src={fireworks} />
        </T.H5C>
      </>
    );
  }
...
const ConfirmedContent = ({ bookingInfo, windowWidth }) => {
  ...
  return (
    <>
      ...
      <CelebrateStatus>
        <div>
          <T.H6C mb="2" mt="2" color="lightGray">
            status
          </T.H6C>
...
const RejectedContent = ({ windowWidth }) => (
  <>
    <Title color="blue" pageTitle windowWidth={windowWidth} inner>
      booking details
    </Title>
...
const AwaitingCancellationContent = ({ windowWidth }) => {
...
const CancelledAfterPaymentContent = ({ internName, windowWidth }) => {
  const history = useHistory();
...
const AutomaticCancelledContent = ({ internName, windowWidth }) => {
  const history = useHistory();
