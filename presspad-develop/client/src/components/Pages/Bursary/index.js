...
const Bursary = ({ windowWidth }) => {
  ...
  const renderUnderConsiderationButton = () => {
    if (bursary) {
      if (bursary.status === 'approved') {
        return (
          <S.BursaryButtonWrapperDesktop>
            <T.H6C color="pink">Good NEWs! You already have a bursary</T.H6C>
            <T.PXS>
              {bursary.discountRate}% bursary, valid from{' '}
              <span style={{ fontWeight: '900' }}>
                {moment(bursary.startDate).format('DD/MM/YYYY')}
              </span>{' '}
              to{' '}
              <span style={{ fontWeight: '900' }}>
                {moment(bursary.endDate).format('DD/MM/YYYY')}
              </span>{' '}
            </T.PXS>
          </S.BursaryButtonWrapperDesktop>
        );
  ...
  return (
    <div>
      ...
      <Row mb="6">
        <Col w={[4, 8, 6]} outer>
          <T.H4C color="pink" mb={3}>
            How does it work?
          </T.H4C>

          <T.P bold color="blue" mb={4} style={{ fontSize: '20px' }}>
            The PressPad bursary is funded directly from our corporate donors,
            grants and any revenue PressPad can generate by delivering training
            or consultancy work. As per our commitment to social impact, we also
            donate 15% of all profits we receive from our hosting platform to
            our bursary impact fund.
          </T.P>

          <T.P color="gray3">
            Our bursary awards cover 50% or 100% of the cost of your placement,
            with the maximum bursary in real money terms capped at £840. We ask
            candidates questions to establish if they may be eligible for a
            bursary award so we can identify those who lack socio-economic
            privilege and are the most in need of our support, which includes
            our widening participation* work. Candidates may apply for multiple
            awards in any given year and PressPad will assess each application
            independently.
          </T.P>
        </Col>
      </Row>

      <T.H4C color="pink" mb={2}>
        FAQS
      </T.H4C>
      <Row inner>
        <Col w={[4, 12, 6]}>
