...
import {
  ADMIN_SINGLE_HOSTS_URL,
  ADMIN_SINGLE_INTERNS_URL,
...
const AdminBookings = () => {
  ...
  const [reFetchBookings, setReFetchBookings] = useState(0);
  ...
  const respondToBooking = (rowData, input) => {
    if (input === 'approve') {
      const {
        price = null,
        hostDetails: { acceptAutomatically = false },
      } = rowData;
      // if host auto accepts and booking price is 0 -> set status to confirmed

      setNewBookingStatus(
        price === 0 && acceptAutomatically === true ? 'confirmed' : 'pending',
      );
  ...
  const activeCols = [
    LinkCol('host', ADMIN_SINGLE_HOSTS_URL, 'id', 'hostDetails._id'),
    LinkCol('intern', ADMIN_SINGLE_INTERNS_URL, 'id', 'internDetails._id'),
    TwoDatesCol('dates'),
    StandardCol('paidByOrganisation', 'perc', null, 'organisation'),
    BursaryCostColumn('bursaryCosts'),
