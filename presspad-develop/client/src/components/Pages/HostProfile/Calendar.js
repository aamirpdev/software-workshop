...
class CalendarComponent extends Component {
  ...
  componentDidMount() {
    ...
    if (bookingSearchDates) {
      const selectedDates = createDatesArray(
        bookingSearchDates[0],
        bookingSearchDates[1],
      );

      const _availableDates = this.refreshAvailableDates(availableDates);
      const _listingActiveBookings = this.refreshListingActiveBookings(
        listingActiveBookings,
      );

      const availableDatesFromSelectedDays = [];

      // eslint-disable-next-line no-restricted-syntax
      for (const date of selectedDates) {
        const isAvailable = this.isThisDateAvailable2({
          date,
          availableDates: _availableDates,
          blockedDates: _listingActiveBookings,
        });

        if (!isAvailable) {
          break;
        } else {
          availableDatesFromSelectedDays.push(date);
        }
      }

      if (availableDatesFromSelectedDays.length) {
        const dates = [
          moment(availableDatesFromSelectedDays[0], 'DD/MM/YYYY').startOf(
            'date',
          ),
          moment(
            availableDatesFromSelectedDays[
              availableDatesFromSelectedDays.length - 1
            ],
            'DD/MM/YYYY',
          ).startOf('date'),
        ];
        stateObj.dates = dates;
        const price = calculatePriceByRange(moment.range(dates));
        stateObj.price = price;
        stateObj.daysAmount = availableDatesFromSelectedDays.length;
        stateObj.isRangeSelected = true;

        this.applyBursary({ price, dates, bursaryData: bursaryData[0] });
      }
    }
    this.setState({
      ...stateObj,
  ...
  isThisDateAvailable2 = ({ date, availableDates, blockedDates }) =>
    !(
  ...
  isThisDateAvailable = ({ date, availableDates, blockedDates }) =>
    !(
      !availableDates.includes(date) ||
      (blockedDates && blockedDates.includes(date)) ||
      moment
  ...
  tileDisabled = ({ date }) => {
    ...
    return !this.isThisDateAvailable({
      date: _date,
      availableDates: avDates,
      blockedDates: listingActiveBookings,
    });
  };
  ...
  handleClick = async () => {
    ...
    try {
      ...
      if (!isComplete) {
        ...
        message =
          "You can't make a request until you get verified. This typically takes 24 hours. If it has been longer than that or your booking is urgent then please contact the PressPad team.";
  ...
  applyBursary = ({ price, dates, bursaryData }) => {
    let _internBursary;

    if (bursaryData) {
      _internBursary = bursaryData;
    } else {
      const { internBursary } = this.state;
      _internBursary = internBursary || {};
    }
  ...
  renderPrice = price => {
    ...
    const _newPrice = formatPrice(price - couponDiscount - bursaryDiscount);
      ...
      <>
        ...
        <BookingRequestDetails>
          ...
          <RequestBtnContainer>
            {isMobile ? (
              <T.PS>
                Price for period <br /> <strong>£{_newPrice}</strong>
              </T.PS>
            ) : (
              <T.PL>
                Price for period <br /> <strong>£{_newPrice}</strong>
