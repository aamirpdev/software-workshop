...
const Verifications = props => {
  ...
  const _validate = async isContinue => {
    ...
    if (
      !(
        state.offerLetter &&
        (state.offerLetter.fileName || state.offerLetter.name)
      ) &&
      ((prevData.offerLetter && prevData.offerLetter.fileName) || isContinue) &&
      !state.hasNoInternship
    ) {
      e = e
          ...
          <Checkbox
            label={
              <>
                ...
                <T.Link to={PARTNERSHIP_AGREEMENT} isExternal color="lightBlue">
                  Partnernship Agreement
                </T.Link>{' '}
              </>
            }
            onChange={e =>
        ...
        <Col w={[4, 12, 12]} mt={5} mb={5}>
          <Checkbox
            label={
              <>
                I have read and agree with the{' '}
                <T.Link to={MEDIA_RELEASE} isExternal color="lightBlue">
                  Media Release Form
                </T.Link>{' '}
              </>
            }
