...
const { bursaryApplicationStatuses: bStatus } = require('../../constants');
  ...
  User.aggregate([
    ...
    },
    {
      $project: {
        ...
        totalBursaryCost: {
          $sum: '$bursaries.totalPotentialAmount',
        },
        previousBursaries: {
          $size: {
            $filter: {
              ...
              cond: {
                $or: [
                  { $eq: ['$$bursary.status', bStatus.rejected] },
                  { $eq: ['$$bursary.status', bStatus.completed] },
                ],
              },
            },
          },
        ...
        currentBursaryStatus: {
          $size: {
            $filter: {
              ...
              cond: {
                $or: [
                  { $eq: ['$$bursary.status', bStatus.request] },
                  { $eq: ['$$bursary.status', bStatus.preApproved] },
                  { $eq: ['$$bursary.status', bStatus.approved] },
                ],
              },
            },
          },
        ...
        totalOrganisationCost: {
          $sum: '$coupons.usedAmount',
          // $sum: '$coupons.reservedAmount',
        },
        coupons: 1,
      },
    },
