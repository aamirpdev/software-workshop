        ...
        .then(async stats => {
          ...
          const cleanStats = await Promise.all(
            stats.map(async intern => {
              ...
              const sortedCoupons = intern.coupons.sort(
              ...
              const internObj = {
                ...
                totalBursaryCost: intern.totalBursaryCost,
                previousBursaries: intern.previousBursaries,
                currentBursaryStatus: !!intern.currentBursaryStatus,
                totalOrganisationCost: intern.totalOrganisationCost,
                latestOrganisationDiscount: sortedCoupons[0]
                  ? `${sortedCoupons[0].discountRate}% - ${sortedCoupons[0].org.name}`
                  : 'N/A',
                status:
          ...
          const cleanStats = await Promise.all(
            stats.map(async host => {
              ...
              if (host.nextLiveBooking) {
                ...
                } else bookingStatus = host.nextLiveBooking.status;
              }
