...
describe('Testing Intern confirming booking when the coupon cover all payments', () => {
  ...
  test('old booking', async done => {
    ...
    const { discountDays } = getDiscountDays({
      ...
      installmentDate: installments.secondUnpaidPaymentNoCoupon.dueDate,
      bookingEnd: endDate,
      couponStart: coupons.activeFull.startDate,
      couponEnd: coupons.activeFull.endDate,
      usedDays: coupons.activeFull.usedDays,
    });

    const couponDiscount =
      (discountDays * 2000 * coupons.activeFull.discountRate) / 100;
    const couponInfo = {
      couponCode: coupons.activeFull.code,
      discountDays,
      discountRate: coupons.activeFull.discountRate,
